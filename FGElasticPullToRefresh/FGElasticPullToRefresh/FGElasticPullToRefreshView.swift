//
//  FGElasticPullToRefreshView.swift
...
// MARK: -
// MARK: FGElasticPullToRefreshState

public
enum FGElasticPullToRefreshState: Int {
    case stopped
    case dragging
    ...
    func isAnyOf(_ values: [FGElasticPullToRefreshState]) -> Bool {
        return values.contains(where: { $0 == self })
    }
}

// MARK: -
// MARK: FGElasticPullToRefreshView

open class FGElasticPullToRefreshView: UIView {
    
    // MARK: -
    // MARK: Vars
    
    fileprivate var _state: FGElasticPullToRefreshState = .stopped
    fileprivate(set) var state: FGElasticPullToRefreshState {
        get { return _state }
        set {
    ...
    var loadingView: FGElasticPullToRefreshLoadingView? {
        willSet {
    ...
    var observing: Bool = false {
        didSet {
            ...
            if observing {
                scrollView.fg_addObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.ContentOffset)
                scrollView.fg_addObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.ContentInset)
                scrollView.fg_addObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.Frame)
                scrollView.fg_addObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.PanGestureRecognizerState)
            } else {
                scrollView.fg_removeObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.ContentOffset)
                scrollView.fg_removeObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.ContentInset)
                scrollView.fg_removeObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.Frame)
                scrollView.fg_removeObserver(self, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.PanGestureRecognizerState)
            }
        }
    }
    ...
    init() {
        ...
        displayLink = CADisplayLink(target: self, selector: #selector(FGElasticPullToRefreshView.displayLinkTick))
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(FGElasticPullToRefreshView.applicationWillEnterForeground), name: UIApplication.willEnterForegroundNotification, object: nil)
    }
    ...
    */
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == FGElasticPullToRefreshConstants.KeyPaths.ContentOffset {
        ...
        } else if keyPath == FGElasticPullToRefreshConstants.KeyPaths.ContentInset {
        ...
        } else if keyPath == FGElasticPullToRefreshConstants.KeyPaths.Frame {
            layoutSubviews()
        } else if keyPath == FGElasticPullToRefreshConstants.KeyPaths.PanGestureRecognizerState {
            if let gestureState = scrollView()?.panGestureRecognizer.state, gestureState.fg_isAnyOf([.ended, .cancelled, .failed]) {
    ...
    fileprivate func currentWaveHeight() -> CGFloat {
        return min(bounds.height / 3.0 * 1.6, FGElasticPullToRefreshConstants.WaveMaxHeight)
    }
    ...
    fileprivate func currentPath() -> CGPath {
        ...
        bezierPath.addLine(to: CGPoint(x: 0.0, y: l3ControlPointView.fg_center(animating).y))
        bezierPath.addCurve(to: l1ControlPointView.fg_center(animating), controlPoint1: l3ControlPointView.fg_center(animating), controlPoint2: l2ControlPointView.fg_center(animating))
        bezierPath.addCurve(to: r1ControlPointView.fg_center(animating), controlPoint1: cControlPointView.fg_center(animating), controlPoint2: r1ControlPointView.fg_center(animating))
        bezierPath.addCurve(to: r3ControlPointView.fg_center(animating), controlPoint1: r1ControlPointView.fg_center(animating), controlPoint2: r2ControlPointView.fg_center(animating))
    ...
    fileprivate func scrollViewDidChangeContentOffset(dragging: Bool) {
        ...
        if state == .stopped && dragging {
            ...
            if offsetY >= FGElasticPullToRefreshConstants.MinOffsetToPull {
                state = .animatingBounce
            } else {
                state = .stopped
            }
        } else if state.isAnyOf([.dragging, .stopped]) {
            let pullProgress: CGFloat = offsetY / FGElasticPullToRefreshConstants.MinOffsetToPull
    ...
    fileprivate func resetScrollViewContentInset(shouldAddObserverWhenFinished: Bool, animated: Bool, completion: (() -> ())?) {
        ...
        if state == .animatingBounce {
            ...
            contentInset.top += FGElasticPullToRefreshConstants.LoadingContentInset
        }
    ...
    fileprivate func animateBounce()
    {
        ...
        let centerY = FGElasticPullToRefreshConstants.LoadingContentInset
        let duration = 0.9
        ...
        UIView.animate(withDuration: duration, delay: 0.0, usingSpringWithDamping: 0.43, initialSpringVelocity: 0.0, options: [], animations: { [weak self] in
            ...
            self?.r3ControlPointView.center.y = centerY
                ...
                if let strongSelf = self, let scrollView = strongSelf.scrollView() {
                    scrollView.fg_addObserver(strongSelf, forKeyPath: FGElasticPullToRefreshConstants.KeyPaths.ContentOffset)
        ...
        UIView.animate(withDuration: duration * 0.4, animations: { [weak self] in
            if let contentInsetTop = self?.originalContentInsetTop {
                self?.bounceAnimationHelperView.center = CGPoint(x: 0.0, y: contentInsetTop + FGElasticPullToRefreshConstants.LoadingContentInset)
            }
            }, completion: nil)
    }
    ...
    @objc func displayLinkTick() {
        ...
        if state == .animatingBounce {
            ...
            scrollView.contentInset.top = bounceAnimationHelperView.fg_center(isAnimating()).y
            scrollView.contentOffset.y = -scrollView.contentInset.top
    ...
    fileprivate func layoutLoadingView() {
        ...
        let loadingViewSize: CGFloat = FGElasticPullToRefreshConstants.LoadingViewSize
        let minOriginY = (FGElasticPullToRefreshConstants.LoadingContentInset - loadingViewSize) / 2.0
